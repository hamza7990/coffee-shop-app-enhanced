...
// Every request is built with an explicit full URL.
// ─────────────────────────────────────────────────────────────
...
class ApiClient {
  ...
  String _url(String path) => Uri.parse('${Config.apiBaseUrl}$path').toString();

  // ── GET ─────────────────────────────────────────────────────
  Future<dynamic> get(String path, {Map<String, dynamic>? queryParams}) async {
    final url = _url(path);
    return _safeRequest(() => _dio.get(url, queryParameters: queryParams));
  }

  // ── POST ────────────────────────────────────────────────────
  Future<dynamic> post(String path, {dynamic body}) async {
    final url = _url(path);
    return _safeRequest(() => _dio.post(url, data: body));
  }

  // ── PUT ─────────────────────────────────────────────────────
  Future<dynamic> put(String path, {dynamic body}) async {
    final url = _url(path);
    return _safeRequest(() => _dio.put(url, data: body));
  }

  // ── PATCH ───────────────────────────────────────────────────
  Future<dynamic> patch(String path, {dynamic body}) async {
    final url = _url(path);
    return _safeRequest(() => _dio.patch(url, data: body));
  }

  // ── DELETE ──────────────────────────────────────────────────
  Future<dynamic> delete(String path) async {
    final url = _url(path);
    return _safeRequest(() => _dio.delete(url));
  }
